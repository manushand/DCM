...
public sealed class ScoringAggregatesTests
{
	...
	private static Scoring BuildScoring(IEnumerable<P> players, Action<ScoringSystem>? configure = null)
	{
		...
		var list = players.Select(static p => new GamePlayer
											  {
												  Power = p.Power,
												  Result = p.Result,
												  Centers = p.Centers,
		...
		for (var pow = GamePlayer.Powers.Austria; pow <= GamePlayer.Powers.Turkey; ++pow)
			if (list.All(g => g.Power != pow))
				list.Add(new () { Power = pow, Result = GamePlayer.Results.Loss, Centers = 0, Years = 0, Other = 0, ProvisionalScore = 0, PlayerAnte = 0 });
	...
	public void UnplayedYears_Based_On_FinalGameYear()
	{
		...
		var prop = typeof (Scoring).GetProperty("PlayerPower", BindingFlags.NonPublic | BindingFlags.Instance)
								   .OrThrow();
