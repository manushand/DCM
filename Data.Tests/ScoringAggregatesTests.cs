...
public sealed class ScoringAggregatesTests
{
	...
	{
		var scoring = BuildScoring([
									   new (Austria, Loss, 5, 8, 0, 0, 0),
									   new (England, Win, 12, 10, 0, 0, 0)
								   ]);
		// For Austria: one higher center count (12), so BestCenterRank=2;
		// WorstCenterRank depends on sharers (none here), so 2
