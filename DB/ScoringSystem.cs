...
internal sealed partial class ScoringSystem : IdentityRecord
{
	...
	internal bool UsesGameResult;
	internal bool UsesYearsPlayed;

	internal string FinalScoreFormula
	{
		get => _finalScoreFormula.TrimEnd(CompiledFormulaSuffix);
		set => _finalScoreFormula = value;
	}

	internal bool UsesProvisionalScore => ProvisionalScoreFormula.Length > 0;
	internal bool UsesPlayerAnte => PlayerAnteFormula.Length > 0;
	internal bool FinalScoreFormulaMissing => RemoveComments(FinalScoreFormula).Length is 0;
	internal bool DrawsAllowed => DrawPermissions is not DrawRules.None;
	internal bool DrawsIncludeAllSurvivors => DrawPermissions is DrawRules.DIAS;
	internal bool UsesOtherScore => OtherScoreAlias.Length > 0;
	internal bool UsesCompiledFormulas => _finalScoreFormula.LastOrDefault() is CompiledFormulaSuffix;
